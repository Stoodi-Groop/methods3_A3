...
#test_data = read.delim("Pitch/Study1D0S101T10_f0.txt")
...
#renaming the coloumn in articulation data
artdata <- artdata  %>% 
    rename(filename = soundname)

#adding descriptors for pitch
data$filename <- as.factor(data$filename)
newdata <- data %>%
  group_by(filename) %>% 
    summarize(meanf0 = mean(V2), 
              sdf0 = sd(V2),
              IQRf0 = IQR(V2))
...
newdata <- newdata %>%
    mutate(Participant = extract_info(newdata, 10, 12), 
           ...
           Diagnosis = extract_info(newdata, 14, 14),
           Trial = str_replace(Trial, "_", ""))

newdata = newdata[,-1]

artdata <- artdata %>% #filename removal of characters
    mutate(filename = extract_info(artdata, 10, 12))

artdata <- artdata  %>% #renaming filename to participant
    rename(Participant = filename)
...
I like to usegroup_by() %>% summarize() for quick summaries
...
```{r}
#the data as csv:
df<-read_csv("clean_port3_data.csv")
df <- df[ , -1] #removing X1 column
df<-na.omit(df) #removing
...
df %>% 
  group_by(Study, Diagnosis) %>% tally()


df$Study<-as.factor(df$Study)
dfstudy1<-filter(df,Study == "1")

dfstudy1 %>% 
  group_by(Diagnosis, SANS) %>% tally()

#look at output and output 2. 
...
library(lme4)
library(lmerTest)
#phonationtime = duration of the recording where speech is present
#speech rate  
...
#preparing for sclaed pause duration column
     set.seed(1)
...
# dfmutated <- dfmutated %>%  
    mutate(pause_duration_scaled = (df$pause_duration - mean(df$pause_duration) / sd(df$pause_duration)))  
