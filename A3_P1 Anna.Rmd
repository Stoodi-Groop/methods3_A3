...
demo<-demo[rep(seq_len(nrow(demo)), each = 10), ] #multiplying the rows 10 times for merge
...
dur <- dur  %>% 
    rename(Participant = soundname)

class(dur$Participant)
dur$Participant<-as.factor(dur$Participant)

dur<- dur %>% #filename removal of characters
    mutate(Participant = extract_info(dur, 10, 12))
