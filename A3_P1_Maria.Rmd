...
singlefile <- read.delim("Pitch/Study1D0S101T1_f0.txt")
...
#test_data = read.table("Pitch/Study1D0S101T1_f0.txt")
...
#load data from the above loop
data <- read.csv("port3_part1_data.csv", header = T)
data = data[-1,] #remove first row (functions as extra header)

#make into numeric (paste is used to not change the exact numbers)
data$V1 <- as.numeric(paste(data$V1))
data$V2 <- as.numeric(paste(data$V2))

#remove na
data <- na.omit(data) #you could consider not doing this but the text tells us to

#filename as factor
data$filename <- as.factor(data$filename)

#grouping dependent on filename, summarize different pitch variables
newdata <- data %>%
  group_by(filename) %>% 
...
#extracting the information needed from filename into own columns
newdata <- newdata %>%
...
#collecting subject names
# subject <- substr(newdata$filename[1:length(newdata$filename)], 10, 12)
# newdata$Participant <- subject

#read in the demographic data 
demo <- read.csv("DemographicData.csv", sep = ";")

#rename diagnosis to 0 and 1
demo <- demo %>%
    mutate(Diagnosis = ifelse(Diagnosis == "Control", "0", "1")) 

artdata <- read.delim("Articulation.txt", sep = ",")



#rename the column soundname to filename in accordance with the other dataframe
artdata <- artdata  %>% 
    rename(filename = soundname) 

#extracting the information needed from filename into own columns
artdata <- artdata %>%  
    mutate(Participant = extract_info(artdata, 10, 12), 
           Study = extract_info(artdata, 6, 6), 
           Trial = extract_info(artdata, 14, 15),
           Diagnosis = extract_info(artdata, 8, 8),
           Trial = str_replace(Trial, "_", ""))
...
#remove na (removes a lot but that's what we are told to do)
df <- df %>% 
  na.omit
...
#describe our sample by diagnosis groups
df %>% 
  split(df$Diagnosis) %>% 
  map(summary)

#filter to divide into studies
study1 <- df %>% 
  filter(Study == 1)
study2 <- df %>% 
  filter(Study == 2)

#describe our sample by diagnosis groups and study
study1 %>% 
  split(df$Diagnosis) %>% 
  map(summary)
...
#NonVerbalIQ
ggplot(df, aes(x = Diagnosis, y = NonVerbalIQ, color = Diagnosis)) +
  geom_boxplot() + 
  facet_wrap(~Study) + 
  theme_bw()
...
#One way of doing it, but we went for another way except for z (see further below)

#    set.seed(1)
#    x <- runif(df$sdPitch)
#    y <- runif(df$phonationtime..s.)
#    z <- runif((df$dur..s.-df$phonationtime..s.)/df$npause)
#    æ <- runif(df$speechrate..nsyll.dur.)
#    
# 
#    # Manually scaling
#    (x - mean(x)) / sd(x)
...
#    (æ - mean(æ)) / sd(æ)
# 
#    scale_sdPitch <- scale(x)
#    scale_phonationtime <- scale(y)
#    scale_pause_dur <- scale(z)
#    scale_speechrate <- scale(æ)


#translations for the new columns we need to make
# pause duration = (ndur - phonationtime / npause)
# speechrate= speechrate 
# proportion of spoken time = phonationtime
# pitch variability = sdPitch
  

   
#mutating the scaled measures used in the models   
df <- df %>%  
...
Now we want to make our models

```{r}
library(lme4)

#make models
#0 means that intercept is control group (makes output more interpretable)
m1 <- lmer(speech_rate_scaled ~ 0 + Study*Diagnosis + (1|ID), df)
m2 <- lmer(proportion_spoken_time_scaled ~ 0 + Study*Diagnosis + (1|ID), df)
m3 <- lmer(Pitch_variability_scaled ~ 0 + Study*Diagnosis + (1|ID), df)
