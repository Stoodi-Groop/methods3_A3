...
```{r}
#omitting NAs so that we are now only working with study 1 and 2
df <- na.omit(df)
...
#subsetting study 1 and 2 to get information by Diagnosis
study1 <- subset(df, Study == 1)
study2 <- subset(df, Study == 2)
...
Making the models
```{r}
m1 <- speech_rate_scaled ~ 0 + Study*Diagnosis + Age + Gender + (1|Participant)
m2 <- proportion_spoken_time_scaled ~ 0 + Study*Diagnosis + Age + Gender + (1|Participant)
m3 <- Pitch_variability_scaled ~ 0 + Study*Diagnosis + Age + Gender + (1|Participant)
m4 <- scale_pause_dur ~ 0 + Study*Diagnosis + Age + Gender + (1|Participant)
```
