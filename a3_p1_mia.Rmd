...
subdata <- newdata[1:20, 1:7]

subject <- substr(newdata$filename[1:length(newdata$filename)], 10, 12)
newdata$Participant <- subject

study <- substr(newdata$filename[1:length(newdata$filename)], 6, 6)
newdata$Study <- study

diagnosis <- substr(newdata$filename[1:length(newdata$filename)], 8, 8)
newdata$Diagnosis <- diagnosis

trial <- substr(newdata$filename[1:length(newdata$filename)], 14, 14)
newdata$Trial <- trial
```
