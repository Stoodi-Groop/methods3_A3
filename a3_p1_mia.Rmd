...
newdata <- data %>%
  group_by(filename) %>% 
    summarize(meanPitch = mean(V2), 
              sdPitch = sd(V2), 
              IQRPitch = IQR(V2), 
              medianPitch = median(V2),
              madPitch = mad(V2), 
              maxPitch = max(V2), 
              minPitch = min(V2))
...
```{r}
df <- read.csv("clean_port3_data.csv", header = T)

#removing NA's - only study 1 and 2 remain
df <- na.omit(df)
...
1#making schizo and control dataframes
scz <- df %>% 
  filter(Diagnosis == 1)
control <- df %>% 
  filter(Diagnosis == 0)
...
df <- df %>%  #mutating the four measures used in the models
    mutate(speech_rate_scaled = 
             (df$speechrate..nsyll.dur. - mean(df$speechrate..nsyll.dur.)) / sd(df$speechrate..nsyll.dur.), 
           proportion_spoken_time_scaled =
             (df$phonationtime..s. - mean(df$phonationtime..s.)/ sd(df$phonationtime..s.)),
           Pitch_variability_scaled = 
             (df$sdPitch - mean(df$sdPitch)) / sd(df$sdPitch))
...
```{r}
#making models
m1 <- lme4::lmer(speech_rate_scaled ~ 0 + Study*Diagnosis + (1|ID), df)
m2 <- lme4::lmer(proportion_spoken_time_scaled ~ 0 + Study*Diagnosis + (1|ID), df)
m3 <- lme4::lmer(Pitch_variability_scaled ~ 0 + Study*Diagnosis + (1|ID), df)
m4 <- lme4::lmer(scale_pause_dur ~ 0 + Study*Diagnosis + (1|ID), df)
